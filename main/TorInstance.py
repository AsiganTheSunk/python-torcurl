...
class TorInstance():
    ...
    def __str__(self):
        return('TorInstance ID: %s\n Nickname: %s\n ProxyPort: %s, CntrlPort: %s\n Circuit Age: %s, Circuit Hops: %s\n ExitPolicy: {%s}\n'
               ' Country List: %s' % (
            self.tor_instance_id, self.nickname, self.proxy_port, self.cntrl_port, self.circuit_age, self.circuit_hops, self.exit_policy,
            self.country_list
        ))
